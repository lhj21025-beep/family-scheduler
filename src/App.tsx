...
type EventItem={id:string;title:string;start:string;end?:string;allDay?:boolean;memberIds:MemberId[];kind?:'event'|'homework'|'homework-complete';homeworkId?:string;completed?:boolean;completedAt?:string;location?:string;memo?:string;repeat?:Repeat;familyId?:string;createdBy?:string}
...
export default function App(){
 ...
 const addSchedule=(arg:any)=>{const start=new Date(arg.date);setModal({id:uid(),title:'',start:dateTime(start),end:dateTime(new Date(start.getTime()+3600000)),memberIds:currentMember?[currentMember]:['me'],allDay:false,kind:'event',repeat:'none'})}
 const clickEvent=(arg:EventClickArg)=>{const e=events.find(x=>x.id===arg.event.id);if(e)setModal(e)}
 const writeEvent=async(e:EventItem)=>{await setDoc(doc(db,'events',e.id),{...e,familyId:FAMILY_ID,createdBy:e.createdBy??auth.currentUser?.uid??''})}
 const saveEvent=async(e:EventItem)=>{if(!e.title.trim())return alert('일정 제목을 입력해주세요.');if(!e.memberIds.length)return alert('최소 한 명을 선택해주세요.');try{const repeat=e.repeat??'none';const dates=repeatDates(e.start,repeat);const duration=e.end?new Date(e.end).getTime()-new Date(e.start).getTime():3600000;await Promise.all(dates.map((d,i)=>writeEvent({...e,id:i===0?e.id:uid(),start:d,end:e.end?dateTime(new Date(new Date(d).getTime()+duration)):undefined,repeat:i===0?repeat:'none'})));setModal(null);flash(repeat==='none'?'일정을 저장했습니다.':`${dates.length}개의 반복 일정을 만들었습니다.`)}catch(error){console.error(error);alert('일정 저장에 실패했습니다. Firebase 연결 또는 Firestore 보안 규칙을 확인해주세요.')}}
 const changeEvent=async(id:string,start:string,end?:string)=>{const old=events.find(x=>x.id===id);if(!old)return;try{await writeEvent({...old,start,end})}catch(error){console.error(error);alert('일정 변경에 실패했습니다.')}}
 const eventDrop=(arg:any)=>{if(arg.event.start)void changeEvent(arg.event.id,dateTime(arg.event.start),arg.event.end?dateTime(arg.event.end):undefined);flash('일정 시간을 변경했습니다.')}
 const eventResize=(arg:any)=>{if(arg.event.start)void changeEvent(arg.event.id,dateTime(arg.event.start),arg.event.end?dateTime(arg.event.end):undefined);flash('일정 시간을 변경했습니다.')}
 const deleteEvent=async(e:EventItem)=>{if(!confirm(`'${e.title}' 일정을 삭제할까요?`))return;try{await deleteDoc(doc(db,'events',e.id));setModal(null);flash('일정을 삭제했습니다.')}catch(error){console.error(error);alert('일정 삭제에 실패했습니다.')}}
 const registerHomework=async()=>{if(!homeworkIds.length)return;const chosen=homeworks.filter(h=>homeworkIds.includes(h.id));const fresh=chosen.filter(h=>!events.some(e=>e.kind==='homework'&&e.homeworkId===h.id&&e.start.slice(0,10)===homeworkDate&&!e.completed));const created=fresh.map(h=>({id:uid(),title:`📝 ${h.name}`,start:`${homeworkDate}T00:00:00`,end:`${homeworkDate}T23:59:59`,allDay:true,memberIds:['son'] as MemberId[],kind:'homework' as const,homeworkId:h.id,completed:false}));if(!created.length)return alert('선택한 숙제는 해당 날짜에 모두 이미 등록되어 있습니다.');try{await Promise.all(created.map(writeEvent));setHomeworkOpen(false);setHomeworkIds([]);flash(`${created.length}개 숙제를 등록했습니다.`)}catch(error){console.error(error);alert('숙제 등록에 실패했습니다.')}}
 const completeHomework=async(e:EventItem)=>{if(e.completed)return;const now=new Date();const next={...e,title:`✅ ${e.title.replace(/^📝\s*/,'')}`,completed:true,completedAt:now.toISOString()};const record:EventItem={id:uid(),title:`✅ 숙제 완료 · ${e.title.replace(/^📝\s*/,'')}`,start:dateTime(now),end:dateTime(new Date(now.getTime()+900000)),memberIds:['son'],kind:'homework-complete',homeworkId:e.homeworkId};try{await Promise.all([writeEvent(next),writeEvent(record)]);setModal(null);flash('숙제를 완료했습니다.')}catch(error){console.error(error);alert('숙제 완료 저장에 실패했습니다.')}}
 const addHomework=async()=>{const n=homeworkName.trim();if(!n)return;if(homeworks.some(h=>h.name===n))return alert('이미 등록된 숙제입니다.');const h={id:uid(),name:n,familyId:FAMILY_ID};try{await setDoc(doc(db,'homeworks',h.id),h);setHomeworkName('')}catch(error){console.error(error);alert('숙제 추가에 실패했습니다.')}}
 ...
 {modal&&<ScheduleModal event={modal} onClose={()=>setModal(null)} onSave={saveEvent} onDelete={deleteEvent} onComplete={completeHomework}/>}</div>
}

function ScheduleModal({event,onClose,onSave,onDelete,onComplete}:{event:EventItem;onClose:()=>void;onSave:(e:EventItem)=>void;onDelete:(e:EventItem)=>void;onComplete:(e:EventItem)=>void}){const [e,setE]=useState(event);if(e.kind==='homework')return <div className="modal-backdrop"><div className="modal homework-complete-modal"><div className="modal-header"><h2>📝 숙제 확인</h2><button onClick={onClose}>×</button></div><div className="homework-detail"><div className={e.completed?'homework-status done':'homework-status'}>{e.completed?'✓ 숙제 완료':'□ 숙제 미완료'}</div><h3>{e.title.replace(/^📝\s*/,'').replace(/^✅\s*/,'')}</h3><p>숙제 날짜: {e.start.slice(0,10)}</p>{e.completed&&e.completedAt&&<p>완료 시간: {new Date(e.completedAt).toLocaleString('ko-KR')}</p>}</div><div className="modal-actions">{!e.completed&&<button className="complete-btn" onClick={()=>onComplete(e)}>✓ 완료</button>}<span/><button onClick={onClose}>닫기</button></div></div></div>
 return <div className="modal-backdrop"><div className="modal"><div className="modal-header"><h2>일정 {event.title?'수정':'추가'}</h2><button onClick={onClose}>×</button></div><label>누구<div className="member-picker">{members.map(m=><button type="button" key={m.id} className={e.memberIds.includes(m.id)?'member-chip selected':'member-chip'} onClick={()=>setE(p=>({...p,memberIds:p.memberIds.includes(m.id)?p.memberIds.filter(x=>x!==m.id):[...p.memberIds,m.id]}))}>{m.icon} {m.label}</button>)}</div></label><label>제목<input autoFocus value={e.title} onChange={x=>setE(p=>({...p,title:x.target.value}))}/></label><div className="form-grid"><label>시작<input type="datetime-local" value={e.start} onChange={x=>setE(p=>({...p,start:x.target.value}))}/></label><label>종료<input type="datetime-local" value={e.end??''} onChange={x=>setE(p=>({...p,end:x.target.value}))}/></label></div><label>반복<select value={e.repeat??'none'} onChange={x=>setE(p=>({...p,repeat:x.target.value as Repeat}))}><option value="none">반복 안 함</option><option value="daily">매일</option><option value="weekdays">평일(월~금)</option><option value="weekly">매주</option><option value="monthly">매월</option></select></label><label>장소<input value={e.location??''} onChange={x=>setE(p=>({...p,location:x.target.value}))}/></label><label>메모<textarea value={e.memo??''} onChange={x=>setE(p=>({...p,memo:x.target.value}))}/></label><div className="modal-actions">{event.title&&<button className="delete-btn" onClick={()=>onDelete(e)}>삭제</button>}<span/><button onClick={onClose}>취소</button><button className="save-btn" onClick={()=>void onSave(e)}>저장</button></div></div></div>}
